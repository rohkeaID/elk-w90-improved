...
!BOP
! !ROUTINE: zfplotw90unk
! !INTERFACE:
subroutine zfplotw90unk(np,vpl,zfmt,zfir,fp)
! !USES:
use modmain
! !INPUT/OUTPUT PARAMETERS:
!   np   : total number of plot points (in,integer)
!   vpl  : 3D plotting points (in,real(3,np))
!   zfmt : complex muffin-tin  function (in,complex(npcmtmax,natmtot))
!   zfir : complex intersitial function (in,complex(ngtot))
!   fp   : evaluated functions at the grid points (out,complex(ngtot,nf))
! !DESCRIPTION:
!   Evaluates {\tt zfmt} and {\tt zfir} functions at the grid points
!
! !REVISION HISTORY:
